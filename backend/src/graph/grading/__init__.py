...
Orchestrates the full grading pipeline: data lookups, Claude attribution
...
def grade_articles(
    ...
    1. Claude attribution classification (parallel, ~500ms per article)
    2. Claude cross-article corroboration (single call, ~1-2s)
    3. Six-factor composite scoring (instant)
    4. Penalties and bonuses (instant)
    5. Claude analyst signal generation (parallel, ~500ms per article)
    ...
    """
    # Lazy imports to avoid loading langchain_anthropic at module level
    ...
    # Step 1: Attribution classification (parallel Claude calls)
    logger.info("Step 1/5: Classifying attribution quality...")
    attribution_scores = classify_attributions_batch(articles)

    # Step 2: Cross-article corroboration (single Claude call)
    ...
    # Step 5: Analyst signal generation (parallel Claude calls)

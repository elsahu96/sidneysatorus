"""
Cross-article corroboration analysis via LLM.
...
_llm = init_chat_model(GRADING_LLM_MODEL, temperature=0, max_tokens=4096)
...
def analyze_corroboration(articles: list[dict]) -> dict[str, dict[str, Any]]:
    ...
    try:
        response = _llm.invoke(_CORROBORATION_PROMPT.format(sources_block=sources_block))
    ...
    # Map domain-level results back to article URLs
